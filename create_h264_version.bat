...
for %%a in (%*) do (
	for /F "delims=" %%i in (%%a) do (
		...
		ffmpeg -hide_banner -y -i %%a -c:a copy -c:s copy -c:v h264_amf -rc cqp -qp_i 18 -qp_p 18 -qp_b 18 "!outfile!"
	)
)
goto exit
...
for %%a in (%*) do (
	for /F "delims=" %%i in (%%a) do (
		set outfile=%%~di%%~pi%%~ni - H264%%~xi
		ffmpeg -hide_banner -y -i %%a -c:a copy -c:v h264_amf -rc cqp -qp_i 18 -qp_p 18 -qp_b 18 "!outfile!"
	)
)
goto exit
